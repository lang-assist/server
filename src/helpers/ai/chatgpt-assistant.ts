...
export class OpenAIModelAssistant extends AIModel {
  constructor(
  ...
  async _init(): Promise<void> {
    ...
    OpenAIModelAssistant.mainInstructionHash = hash;
  ...
  async prepareThread(args: {
    ...
    if (!args.journey) {
      ...
      if (
        !journey.assistantId ||
        OpenAIModelAssistant.mainInstructionHash !== journey.assistantHash
      ) {
        ...
        const newJourney = await JourneyHelper.updateJourney(journey._id, {
          assistantId: assistant.id,
          assistantHash: OpenAIModelAssistant.mainInstructionHash!,
        });
  ...
  async _generateConversationTurn(
    ...
    try {
      ...
      const run = this.client.beta.threads.runs.stream(thread.id, {
        assistant_id: OpenAIModelAssistant.conversationAssistantId!,
        additional_messages: messages,
        additional_instructions: context,
      });
  ...
  async runConversationThread(args: {
    ...
    journey_ID: ObjectId;
    path_ID: ObjectId;
  }): Promise<{ res: AIConversationTurnResponse; usage: AIUsage }> {
    let runId: string | null = null;
    let stepId: string | null = null;
    let msgId: string | null = null;

    const resText = await new Promise<{
      res: AIConversationTurnResponse;
      usage: AIUsage;
    }>(async (resolve, reject) => {
      try {
        let usage: AIUsage | null = null;
        let parsed: AIConversationTurnResponse | null = null;
            ...
            case "thread.message.completed":
              ...
              if (content.type === "text") {
                try {
                  ...
                  if (usage) {
                    resolve({
                      res: parsed as AIConversationTurnResponse,
                      usage: usage ?? {
                        input: 0,
                        output: 0,
                        cachedInput: 0,
  ...
  async removeAssistant() {
    ...
    OpenAIModelAssistant.conversationAssistantId = null;
  }
  ...
  async _getInitialTemplateAssistant(language: SupportedLocale) {
    if (OpenAIModelAssistant._initialTemplateAssistants[language]) {
      return OpenAIModelAssistant._initialTemplateAssistants[language]
        .assistantId;
    }
    ...
    const assistant = await this.client.beta.assistants.create({
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: aiReviewResponseSchema(["newMaterials"]) as any,
        },
      },
    });

    OpenAIModelAssistant._initialTemplateAssistants[language] = {
      assistantId: assistant.id,
      hash,
    };
  ...
  async _createConversationAssistant() {
    if (OpenAIModelAssistant.conversationAssistantId) {
      return;
    }
    ...
    if (meta && meta.hash === hash) {
      OpenAIModelAssistant.conversationAssistantId = meta.assistantId as string;
      return;
    }
    ...
    OpenAIModelAssistant.conversationAssistantId = assistant.id;
