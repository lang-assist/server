...
export class OpenAIModelAssistant extends AIModel<ChatGeneration<any>> {
  async _generate(
  ...
  constructor(public modelName: string, price: BrocaTypes.AI.Types.AIPricing) {
    super("chat", price);
    this.name = modelName;
  }
    ...
    try {
      ...
      if (assistant && assistant.version !== assistantContext.version) {
        ...
        const nAssistant = await this.client.beta.assistants.create({
          ...
          response_format: {
            type: "json_schema",
            json_schema: gen.schema.schema as any,
          },
        });
